...
bool Solution::isValid(vector<string> &strs)
{
  return !((typeid(strs) != typeid(vector<string>)) || (strs.size() == 0));
};

int Solution::getMinLen(vector<string> &strs)
{
...
bool Solution::isCommonPrefix(int mid, vector<string> &strs)
{
...
bool Solution::startsWith(string str, string match, int pos)
{
